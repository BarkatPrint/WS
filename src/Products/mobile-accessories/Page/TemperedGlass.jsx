...
export default function TemperedGlass() {
  ...
  const handleBuyNow = (product) => {
    pauseAutoScroll();
    const selectedBrand = brandSelections[product.id]?.brand || "Not specified";
    const customBrand = brandSelections[product.id]?.custom || "";
...
🏷️ Mobile Brand: ${finalBrand}`;
  ...
  const handleBrandChange = (productId, brand) => {
  ...
  const handleCustomBrandChange = (productId, custom) => {
    setBrandSelections((prev) => ({
      ...prev,
      [productId]: { ...prev[productId], custom },
    }));
  };
      ...
      <div className="relative flex items-center">
        ...
        <div ref={scrollRef} className="flex overflow-x-auto scrollbar-hide space-x-4 pb-4 px-8 snap-x snap-mandatory">
          {products.map((product) => {
            ...
            const selectedBrand = brandSelections[product.id]?.brand || "Vivo";
            const customBrand = brandSelections[product.id]?.custom || "";
            ...
            return (
              <div key={product.id} className="min-w-[220px] max-w-[220px] snap-start border rounded-xl shadow-lg p-4 bg-white flex flex-col">
                ...
                <label className="text-sm font-medium text-gray-700 mb-1">Select Mobile Brand:</label>
                <select
                  value={selectedBrand}
